/* Parham K.
3.3 V logic used.  3-wire SPI. ST7789S controller
The purpose of this is to show different types of iniation.
There is also the option to use or not use port maniuplation.*/
...
void SPI_WriteComm(unsigned char a){
  ...
  //Then we will make MOSI send one or zero
  for(int i; i < 8; i++){
    if(a&mask >> 7 == 1) {
...
void SPI_WriteDataFast(unsigned char c){
  ...
  for (i=0;i<8;i++){
    // Clock toggled low
    PORTD &= ~0x80;
	//Check if mask is being checked against
    if((c&mask)>>7 == 1){
	...
	//Clock toggled high
    PORTD |= 0x80;
	//Byte shifted to be checked against masked.  then it will be pushed after masking-check
    c = c << 1;
  }
  PORTD |= 0x10;
}
